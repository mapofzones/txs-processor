...
func Test_addChannels(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			"empty_args",
			args{},
			"insert into ibc_channels(zone, channel_id, connection_id, is_opened, counterparty_channel_id) values \n    on conflict(zone, channel_id) do nothing;",
		},
		{
			"first_args",
			args{"origin1", map[string]map[string]string{"channelID1": {"connectionID1": "counterpartyChannelID1"}}},
			"insert into ibc_channels(zone, channel_id, connection_id, is_opened, counterparty_channel_id) values ('origin1', 'channelID1', 'connectionID1', false, 'counterpartyChannelID1')\n    on conflict(zone, channel_id) do nothing;",
		},
		{
			"second_args",
			args{"origin2", map[string]map[string]string{"channelID2": {"connectionID2": "counterpartyChannelID2"}}},
			"insert into ibc_channels(zone, channel_id, connection_id, is_opened, counterparty_channel_id) values ('origin2', 'channelID2', 'connectionID2', false, 'counterpartyChannelID2')\n    on conflict(zone, channel_id) do nothing;",
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
...
func Test_markChannel(t *testing.T) {
	...
	tests := []struct {
		...
		{
			"first_args",
			args{"origin1", "myChannelID1", true, "myCounterpartyChannelId1"},
			"update ibc_channels\n    set is_opened = true , counterparty_channel_id = 'myCounterpartyChannelId1'\n        where zone = 'origin1'\n        and channel_id = 'myChannelID1';",
		},
		{
			"second_args",
			args{"origin2", "myChannelID2", false, "myCounterpartyChannelId2"},
			"update ibc_channels\n    set is_opened = false , counterparty_channel_id = 'myCounterpartyChannelId2'\n        where zone = 'origin2'\n        and channel_id = 'myChannelID2';",
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			actual := markChannel(tt.args.origin, tt.args.channelID, tt.args.state, tt.args.counterpartyChannel)
			assert.Equal(t, tt.expected, actual)
		})
	}
}
